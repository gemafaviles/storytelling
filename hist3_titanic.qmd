...
```{r}
#| code-summary: Lectura de datos de los datos titanic_train del paquete titanic
...
Recuento y seleción de variables
```{r}
...
¿Qué información contiene la variable `clase`?
```{r}
#| code-summary: Descipción
titanic_conteo |> describe(clase, n = n)
```


```{r}
#| code-summary: Gráfico de barras para variables cualitativas
titanic_conteo |> explore(clase, n = n)
```


```{r}
#| code-summary: ¿Cuántos sobrevivieron de cada clase?
...
Lo normal es que queramos la información de todas las variables del dataset, en este caso, del objeto `titanic_conteo`.
```{r}
...
titanic <- use_data_titanic(count = _____) # ahora trabajamos con los datos "crudos", sin conteo
...
titanic <- use_data_titanic(count = FALSE) # ahora trabajamos con los datos "crudos", sin conteo
names(titanic) <- c("clase", "sexo", "edad", "supervivencia")
