...
Además, tenemos que saber muy bien qué es lo que queremos obtener como producto, en este caso para mi primo al que, aunque lo voy a hacer por amor al arte, tengo que ser capaz de localizarle sitios con menos contaminación.
...
Ya hemos visto paquetes como `skimr`, `DataExplorer` y `dlookr` que generan informes automáticos con los principales descriptivos. Son muy útiles y merece la pena conocerlos, porque vemos la estructura de los datos y la relación entre las variables mucho más deprisa.
:::
...
plot_contam_mad <- contam_mad |>
  ...
  scale_x_continuous(date_labels = "%y")
  }) +
  theme_minimal() +
