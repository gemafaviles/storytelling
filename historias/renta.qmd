...
Me ha pedido el jefe que le haga un informe sobre la renta municipal en España de los últimos años. Me ha pasado unos datos que ha sacado de la web del INE y ha metido en el paquete `CDR`. Me ha dicho que los datos están limpios, pero no me fío. Vamos a hacer un análisis exploratorio para ver si hay algo raro. Y le preparo un par de gráficos para que vea que soy un crack.

## Lectura de datos

Instalamos los paquetes necesarios si no lo están ya y cargamos los datos...

```{r}
if(!require("remotes")) {install.packages("remotes")}
if(!require("CDR")) {remotes::install_github("cdr-book/CDR")}
library(CDR)
...
head(renta)
```
¡Esto ya es otra cosa! Ahora sí que podemos empezar a trabajar con datos limpios.
...
p_renta_2019 <- ggplot(renta, aes(`2019`)) +
  ...
  geom_vline(aes(xintercept = mean(`2019`)), color = "red", linetype = "dashed", linewidth = 1) +
  geom_text(aes(x = mean(`2019`) + 1400, y = 100, label = round(mean(`2019`), 2)), color = "darkblue") +
...
p_renta_2019_density <- ggplot(renta, aes(`2019`)) +
  ...
  geom_vline(aes(xintercept = mean(`2019`)), color = "red",
             lwd = 1, linetype = "dashed") +
  geom_text(aes(x = mean(`2019`), y = 0.00001, label = round(mean(`2019`), 2)), color = "darkblue") +
  labs(title = "Distribución de la renta por municipios",
       x = "Renta per cápita",
       y = "Densidad") +
  theme_minimal()
...
p_renta_2019_density + 
  geom_histogram(aes(y=..density..), 
                 fill = "lightgreen", color="blue", alpha = 0.4)
```
...
violin <- p + geom_violin(color = "darkblue", fill = "lightblue") +
  theme_minimal()
...
medias <- renta |>
  ...
  pivot_longer(cols = `2015`:`2019`, names_to = "pos",
               values_to = "renta") |>
...
p + geom_violin( ) +
     geom_violin(aes(x=0, y= `2015`),
                 fill = "lightblue", col = "transparent") +
     geom_violin(aes(x=1, y= `2016`),
                 fill = "lightblue", col = "transparent") +
     geom_violin(aes(x=2, y= `2017`),
                 fill = "lightblue", col = "transparent") +
     geom_violin(aes(x=3, y= `2018`),
                 fill = "lightblue", col = "transparent") +
     geom_violin(aes(x=4, y= `2019`),
                 fill = "lightblue", col = "transparent") +
  # añade la media de cada año al violín
  geom_point(aes(x=0, y=mean(`2015`)), color = "darkblue") +
  geom_point(aes(x=1, y=mean(`2016`)), color = "darkblue") +
  geom_point(aes(x=2, y=mean(`2017`)), color = "darkblue") +
  geom_point(aes(x=3, y=mean(`2018`)), color = "darkblue") +
  geom_point(aes(x=4, y=mean(`2019`)), color = "darkblue") +
  # añade la línea de tendencia uniendo las medias
  geom_line(data = medias,
            aes(x = pos, y = renta, group = 1),
            color = "darkblue") +
  theme_minimal() +
    scale_x_continuous(breaks = c(0, 1, 2, 3, 4),
          labels = c("2015", "2016", "2017", "2018", "2019")) +
...
# Optativo, lollipop por provincias
...
## ¿Y si ordenamos el lollipop por provincias? Y de nuevo, ¿nos aporta el color?
