...
**Paso 5** Haz un resumen esploratorio rápido y global con la función `skim()` del paquete `skimr`
...
**Paso 5** 👶 Analiza la variable `Survived` teniendo en cuenta la edad del pasajero `Age` mediante un gráfico de violín con la función `geom_violin()` y a. Considera la variable `x = Survived` e `y = Age`.
...
**Paso 3** Creamos un objeto `tabla_titanic` con las variables objeto de interés
...
**Paso 3** Representamos el diagrama de flujo. Funciones importantes: `geom_alluvium()`, que añade los flujos y `geom_stratum()` que añade los estratos o categorías.
